-- Migração 10: transferência de posse do espaço que funciona (função transfer_space_ownership).
--
-- Problema: o app transferia a posse em 3 passos soltos (spaces.creator_id, e o cargo de cada um em space_members):
--  * a policy "owners update spaces" exige creator_id = auth.uid() também na linha NOVA, então o dono nunca
--    conseguia trocar o creator_id para outra pessoa (erro de RLS);
--  * space_members não tem policy de UPDATE: as mudanças de cargo eram descartadas em silêncio (0 linhas, sem erro).
--
-- Solução: uma função SECURITY DEFINER que confere quem chama e faz tudo de uma vez (atômico).
-- Não abre nenhuma policy nova: space_members continua sem UPDATE direto para os usuários.
--
-- Reversão: rollback_10_transfer_space_ownership.sql
...
begin
  if v_uid is null or not public.is_space_creator(p_space_id) then
    raise exception 'Só o dono do espaço pode transferir a posse.' using errcode = '42501';
  end if;
  ...
  if not exists (
    ...
    raise exception 'A pessoa escolhida não é membro deste espaço.' using errcode = '22023';
  end if;
